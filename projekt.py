...
def main():
    ...
    while cfg.vid_przod.isOpened() and cfg.vid_bok.isOpened():
        ...
        if not ret_przod or not ret_bok or frame_przod is None or frame_bok is None:
            continue
        ...
        data_przod = calc.get_data(wynik_przod.pose_landmarks, w_p, h_p) \
            if (wynik_przod is not None and wynik_przod.pose_landmarks) else None
        data_bok = calc.get_data(wynik_bok.pose_landmarks, w_b, h_b) \
            if (wynik_bok is not None and wynik_bok.pose_landmarks) else None

        if wynik_przod is not None and wynik_przod.pose_landmarks:
            cfg.mp_drawing.draw_landmarks(
                frame_przod, wynik_przod.pose_landmarks, cfg.mp_holistic.POSE_CONNECTIONS)
        if wynik_bok is not None and wynik_bok.pose_landmarks:
            cfg.mp_drawing.draw_landmarks(
        ...
        if poprzedni_stan_ruchu == aktualny_cel and \
            ...
            q_ok = (max_punkty_przod >= 70) and (max_punkty_bok >= 70)
            ...
            if q_ok:
                ...
                for fb in all_fb[:2]:
                    threading.Thread(target=log_error, args=(user_name, fb), daemon=True).start()
